...
final class ICloudDupesModel: ObservableObject {
    ...
    private nonisolated static let root = FileManager.default.homeDirectoryForCurrentUser
    ...
    func scan() {
        ...
        Task {
            ...
            progress = t("listing iCloud Drive…")
            fraction = nil
            ...
            AppLog.log("ICLOUD: \(inventory.files.count) ficheros, \(toHash.count) candidatos (\(formatBytes(totalBytes)) a hashear), \(inventory.skipped) sin descargar")
            ...
            scanning = false
            scanned = true
            AppLog.log("ICLOUD: \(groups.count) grupos duplicados, \(formatBytes(wastedTotal)) desperdiciados")
            SoundFX.shared.play(.done)
        }
    }

    nonisolated static func listFiles() -> (files: [(path: String, size: Int64)], skipped: Int) {
